        ...
        if step_id == "step-1":
            return {
                ...
                "sql": (
                    ...
                    "GROUP BY td_id, transaction_state LIMIT 2"
                ),
                "assumptions": [],
            }
        return {
            "type": "sql_ready",
            "sql": (
                "SELECT td_id, SUM(repeat_spend) AS repeat_spend, SUM(new_spend) AS new_spend "
                "FROM cia_sales_insights_cortex GROUP BY td_id LIMIT 2"
            ),
            "assumptions": [],
        }
