from __future__ import annotations

from app.models import AgentResponse, ChatTurnRequest, DataTable, EvidenceRow, QueryPlanStep, SqlExecutionResult, ValidationResult


def _table_from_evidence(id_: str, name: str, rows: list[EvidenceRow], source_sql: str) -> DataTable:
    return DataTable(
        id=id_,
        name=name,
        columns=["segment", "prior", "current", "changeBps", "contribution"],
        rows=[
            {
                "segment": row.segment,
                "prior": row.prior,
                "current": row.current,
                "changeBps": row.changeBps,
                "contribution": row.contribution,
            }
            for row in rows
        ],
        rowCount=len(rows),
        description="Segment-level decomposition used in the analysis narrative.",
        sourceSql=source_sql,
    )


def _tables_from_sql_results(results: list[SqlExecutionResult]) -> list[DataTable]:
    tables: list[DataTable] = []
    for index, result in enumerate(results, start=1):
        columns = list(result.rows[0].keys()) if result.rows else []
        tables.append(
            DataTable(
                id=f"sql_step_{index}",
                name=f"SQL Step {index} Output",
                columns=columns,
                rows=result.rows,
                rowCount=result.rowCount,
                sourceSql=result.sql,
            )
        )
    return tables


async def mock_classify_route(request: ChatTurnRequest) -> str:
    query = request.message.lower()
    if any(keyword in query for keyword in ["why", "driver", "compare", "versus"]):
        return "deep_path"
    return "fast_path"


async def mock_create_plan(request: ChatTurnRequest) -> list[QueryPlanStep]:
    route = await mock_classify_route(request)
    if route == "fast_path":
        return [QueryPlanStep(id="step_1", goal="Retrieve primary KPI by requested segment and period")]

    return [
        QueryPlanStep(id="step_1", goal="Retrieve primary KPI over selected period"),
        QueryPlanStep(id="step_2", goal="Retrieve segment-level driver decomposition"),
        QueryPlanStep(id="step_3", goal="Compute concentration and severity deltas"),
    ]


async def mock_run_sql(_request: ChatTurnRequest, plan: list[QueryPlanStep]) -> list[SqlExecutionResult]:
    results: list[SqlExecutionResult] = []
    for index, step in enumerate(plan, start=1):
        results.append(
            SqlExecutionResult(
                sql=f"SELECT segment, prior, current, delta_bps, contribution FROM mock_result_set_{index} -- {step.goal}",
                rows=[
                    {
                        "segment": "North America - Unsecured",
                        "prior": 1.82,
                        "current": 2.43,
                        "delta_bps": 61,
                        "contribution": 0.56,
                    },
                    {
                        "segment": "North America - Secured",
                        "prior": 1.09,
                        "current": 1.21,
                        "delta_bps": 12,
                        "contribution": 0.09,
                    },
                    {
                        "segment": "EMEA - Unsecured",
                        "prior": 1.58,
                        "current": 1.88,
                        "delta_bps": 30,
                        "contribution": 0.21,
                    },
                    {
                        "segment": "APAC - Unsecured",
                        "prior": 1.44,
                        "current": 1.49,
                        "delta_bps": 5,
                        "contribution": 0.03,
                    },
                ],
                rowCount=4,
            )
        )
    return results


async def mock_validate_results(results: list[SqlExecutionResult]) -> ValidationResult:
    return ValidationResult(
        passed=True,
        checks=[
            f"Executed {len(results)} governed SQL step(s)",
            "Subtotal reconciliation <= 0.5 bps",
            "Null-rate threshold < 1%",
        ],
    )


async def mock_build_response(request: ChatTurnRequest, results: list[SqlExecutionResult]) -> AgentResponse:
    query = request.message.lower()
    is_fraud = "fraud" in query or "dispute" in query
    is_deposit = "deposit" in query or "liquidity" in query

    if is_fraud:
        evidence = [
            EvidenceRow(segment="NA - CNP", prior=0.82, current=1.26, changeBps=44, contribution=0.61),
            EvidenceRow(segment="NA - Card Present", prior=0.33, current=0.42, changeBps=9, contribution=0.08),
            EvidenceRow(segment="EMEA - CNP", prior=0.55, current=0.68, changeBps=13, contribution=0.14),
        ]

        return AgentResponse(
            answer=(
                "Fraud loss rate rose 28 bps quarter-over-quarter, with card-not-present traffic in North America "
                "driving most of the increase."
            ),
            confidence="high",
            whyItMatters=(
                "The pattern is corridor-concentrated and therefore operationally actionable. Prioritizing top "
                "merchant corridors should reduce losses quickly."
            ),
            metrics=[
                {"label": "Fraud Loss Rate", "value": 1.31, "delta": 28, "unit": "pct"},
                {"label": "High-Risk Corridor Share", "value": 42, "delta": 7, "unit": "pct"},
                {"label": "Recovered Disputes", "value": 0.74, "delta": -0.11, "unit": "usd"},
            ],
            evidence=evidence,
            insights=[
                {
                    "id": "i1",
                    "title": "Losses are corridor-concentrated",
                    "detail": "Top 3 merchant corridors now account for 64% of incremental fraud losses.",
                    "importance": "high",
                },
                {
                    "id": "i2",
                    "title": "Chargeback recovery deteriorated",
                    "detail": "Recovery dropped 11% QoQ, amplifying net fraud pressure.",
                    "importance": "medium",
                },
            ],
            suggestedQuestions=[
                "Which merchant corridors generated the highest incremental losses?",
                "How much of the increase is volume versus severity?",
                "What controls have the fastest expected loss reduction?",
            ],
            assumptions=[
                "Fraud rate uses confirmed-loss basis and excludes pending investigations.",
                "Quarter is based on settlement date.",
            ],
            trace=[
                {
                    "id": "t1",
                    "title": "Resolve intent and scope",
                    "summary": "Mapped question to governed fraud metrics, segments, and time window.",
                    "status": "done",
                    "qualityChecks": ["Metric dictionary hit", "Ambiguity check passed"],
                },
                {
                    "id": "t2",
                    "title": "Generate governed SQL",
                    "summary": "Generated SQL against allowlisted fraud summary tables.",
                    "status": "done",
                    "sql": results[0].sql if results else None,
                    "qualityChecks": ["Join graph constraint passed", "No restricted columns accessed"],
                },
            ],
            dataTables=[
                _table_from_evidence(
                    "fraud_drivers",
                    "Fraud Driver Breakdown",
                    evidence,
                    "SELECT region_channel AS segment, prior_rate AS prior, current_rate AS current, delta_bps AS changeBps, contribution_share AS contribution FROM curated.fraud_summary WHERE quarter IN ('2025Q3','2025Q4');",
                ),
                *_tables_from_sql_results(results),
            ],
        )

    if is_deposit:
        evidence = [
            EvidenceRow(
                segment="Commercial - Rate Sensitive",
                prior=5.1,
                current=2.4,
                changeBps=-270,
                contribution=-0.44,
            ),
            EvidenceRow(segment="Retail - Transactional", prior=2.2, current=2.9, changeBps=70, contribution=0.18),
            EvidenceRow(segment="Wealth - Sweep", prior=1.8, current=1.3, changeBps=-50, contribution=-0.07),
        ]

        return AgentResponse(
            answer=(
                "Net deposit growth slowed to 2.1% in the latest quarter, driven by commercial outflows in "
                "rate-sensitive accounts."
            ),
            confidence="medium",
            whyItMatters="Liquidity coverage remains healthy, but funding mix shift may pressure NIM if this persists.",
            metrics=[
                {"label": "Net Deposit Growth", "value": 2.1, "delta": -1.8, "unit": "pct"},
                {"label": "Commercial Outflow", "value": 1.4, "delta": 0.6, "unit": "usd"},
                {"label": "LCR Buffer", "value": 129, "delta": -4, "unit": "pct"},
            ],
            evidence=evidence,
            insights=[
                {
                    "id": "i1",
                    "title": "Pricing sensitivity is concentrated",
                    "detail": "Outflows cluster in accounts repriced within 45 days.",
                    "importance": "high",
                },
                {
                    "id": "i2",
                    "title": "Retail balances partially offset",
                    "detail": "Transactional inflows offset 27% of commercial attrition.",
                    "importance": "medium",
                },
            ],
            suggestedQuestions=[
                "Which client cohorts show the highest repricing sensitivity?",
                "How does mix shift affect forward NIM scenarios?",
                "What is the projected liquidity impact if trend persists 2 more quarters?",
            ],
            assumptions=[
                "Deposit growth is period-end average balance basis.",
                "Commercial segment excludes brokered deposits.",
            ],
            trace=[
                {
                    "id": "t1",
                    "title": "Resolve intent and scope",
                    "summary": "Mapped question to liquidity and funding mix definitions.",
                    "status": "done",
                    "qualityChecks": ["Time window recognized", "Metric dictionary hit"],
                },
                {
                    "id": "t2",
                    "title": "Generate governed SQL",
                    "summary": "Generated SQL against curated deposit mix summary.",
                    "status": "done",
                    "sql": results[0].sql if results else None,
                    "qualityChecks": ["Allowlist enforced", "PII policy check passed"],
                },
            ],
            dataTables=[
                _table_from_evidence(
                    "deposit_mix",
                    "Deposit Mix Shift",
                    evidence,
                    "SELECT client_segment AS segment, prior_growth AS prior, current_growth AS current, delta_bps AS changeBps, contribution_share AS contribution FROM curated.deposit_mix_summary WHERE quarter IN ('2025Q3','2025Q4');",
                ),
                *_tables_from_sql_results(results),
            ],
        )

    evidence = [
        EvidenceRow(segment="North America - Unsecured", prior=1.82, current=2.43, changeBps=61, contribution=0.56),
        EvidenceRow(segment="North America - Secured", prior=1.09, current=1.21, changeBps=12, contribution=0.09),
        EvidenceRow(segment="EMEA - Unsecured", prior=1.58, current=1.88, changeBps=30, contribution=0.21),
        EvidenceRow(segment="APAC - Unsecured", prior=1.44, current=1.49, changeBps=5, contribution=0.03),
    ]

    return AgentResponse(
        answer=(
            "Charge-off rate increased 42 bps quarter-over-quarter, with North America unsecured cards "
            "contributing most of the deterioration."
        ),
        confidence="high",
        whyItMatters=(
            "The increase is concentrated rather than broad-based, enabling targeted mitigation in underwriting "
            "and collections."
        ),
        metrics=[
            {"label": "Portfolio Charge-Off Rate", "value": 2.42, "delta": 42, "unit": "pct"},
            {"label": "Quarter-over-Quarter Change", "value": 42, "delta": 42, "unit": "bps"},
            {"label": "At-Risk Balance", "value": 4.8, "delta": 0.6, "unit": "usd"},
        ],
        evidence=evidence,
        insights=[
            {
                "id": "i1",
                "title": "Concentration risk is rising",
                "detail": "North America unsecured now represents 56% of worsening while only 38% of exposure.",
                "importance": "high",
            },
            {
                "id": "i2",
                "title": "Severity outran volume",
                "detail": "Default severity rose faster than default counts, suggesting collections efficacy drift.",
                "importance": "high",
            },
            {
                "id": "i3",
                "title": "APAC stayed resilient",
                "detail": "APAC movement remains low and can be a policy benchmark.",
                "importance": "medium",
            },
        ],
        suggestedQuestions=[
            "Break down NA unsecured by FICO band and vintage.",
            "How much of the increase came from new originations versus existing book?",
            "Which interventions reduced severity in similar historical periods?",
        ],
        assumptions=[
            "Rates shown as balance-weighted portfolio rates.",
            "Comparisons use quarter-end snapshots aligned to finance calendar.",
        ],
        trace=[
            {
                "id": "t1",
                "title": "Resolve intent and scope",
                "summary": "Mapped request to credit risk semantic model entities, metrics, and period constraints.",
                "status": "done",
                "qualityChecks": ["Metric dictionary hit", "Time window recognized"],
            },
            {
                "id": "t2",
                "title": "Generate governed SQL",
                "summary": "Built allowlisted SQL against curated credit risk tables with controlled joins.",
                "status": "done",
                "sql": results[0].sql if results else None,
                "qualityChecks": ["Join graph constraint passed", "No restricted columns accessed"],
            },
            {
                "id": "t3",
                "title": "Validate and rank insights",
                "summary": "Reconciled totals and ranked findings by impact x confidence x user relevance.",
                "status": "done",
                "qualityChecks": ["Subtotal reconciliation <= 0.5 bps", "Null-rate threshold < 1%"],
            },
        ],
        dataTables=[
            _table_from_evidence(
                "charge_off_drivers",
                "Charge-Off Driver Breakdown",
                evidence,
                "SELECT region || ' - ' || product_type AS segment, q3_rate AS prior, q4_rate AS current, (q4_rate-q3_rate)*10000 AS changeBps, contribution_share AS contribution FROM curated.credit_risk_summary WHERE quarter IN ('2025Q3','2025Q4');",
            ),
            *_tables_from_sql_results(results),
        ],
    )
